...
export function PatientProfileModal({
  ...
  if (patients.length === 0) {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
        <div
          className="w-full max-w-md bg-vm-card rounded-xl shadow-2xl border border-vm-border overflow-hidden animate-fade-in-up"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between px-6 py-4 border-b border-vm-border bg-vm-surface">
            <h2 className="font-display text-lg font-semibold text-vm-text">
              No Patient Found
            </h2>
            <button
              onClick={onClose}
              className="p-1 rounded-md hover:bg-vm-bg text-vm-text-tertiary hover:text-vm-text-secondary transition-colors"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
      <div
        className="w-full max-w-2xl bg-vm-card rounded-xl shadow-2xl border border-vm-border overflow-hidden flex flex-col max-h-[85vh] animate-fade-in-up"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-vm-border bg-vm-surface">
